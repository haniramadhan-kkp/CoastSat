...
# if len(sys.argv) < 2:
#     print("Usage: python igtabrasi.py <config_path>")
#     sys.exit(1)

# config_path = sys.argv[1]
config_path = "igtabrasi.yaml"
...
print(f"[INFO] Running CoastSat for polygon: {config['sitename']}")
print(f"[INFO] Date range: {config['dates'][0]} to {config['dates'][1]}")
print(f"[INFO] Satellites: {config['sat_list']}")
...
# region of interest
kml_polygon = os.path.join(os.getcwd(), 'igtabrasi', str(sitename), str(sitename) + '.kml')
...
# Tambahkan key opsional jika ada di config
if 'LandsatWRS' in config:
    inputs['LandsatWRS'] = config['LandsatWRS']

if 'S2tile' in config:
    inputs['S2tile'] = config['S2tile']
